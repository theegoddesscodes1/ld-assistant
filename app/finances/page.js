...
const BUSINESSES = ["Lilac Desk", "Fiverr"];
...
export default function FinancesPage() {
  ...
  useEffect(() => {
    fetch("/api/finances").then((r) => r.json()).then((d) => setTransactions(d.transactions || []));
  ...
  const setAside = savedPercent != null ? (totalIncome * savedPercent) / 100 : null;
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto", padding: "36px 24px 72px 24px" }}>
      <PageHeader title="Finances" subtitle={`Net ${money(net)} logged so far.`} />

      <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 20 }}>
        <StatTile label="Total Income" value={money(totalIncome)} />
